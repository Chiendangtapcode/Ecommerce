...
import { decrementQty, deleteItem, incrementQty, resetCart } from '../redux/ecommerceSlice'
import { emtyCart } from '../assets/index'
                    ...
                    <div className='container mx-auto h-auto grid grid-cols-5 gap-8'>
                        <div className='w-full h-full bg-white px-4 col-span-4'>
                                        ...
                                        <div key={item.id} className='w-full border-b-[1px] border-b-gray-300 p-4 flex items-center gap-6'>
                                            <div className='w-full flex items-center justify-center gap-6'>
                                                ...
                                                <div className='w-3/5'>
                                                    ...
                                                    <div className='bg-[#f0f2f2] flex justify-center gap-1 w-36 py-1 text-center drop-shadow-lg rounded-md'>
                                                        <p>Quantity:</p>
                                                        <p onClick={() => dispatch(decrementQty(item.id))} className='cursor-pointer bg-gray-200 px-1 rounded-md hover:bg-gray-400 duration-300'>-</p>
                                                        <p>{item.quantity}</p>
                                                        <p onClick={() => dispatch(incrementQty(item.id))} className='cursor-pointer bg-gray-200 px-1 rounded-md hover:bg-gray-400 duration-300'>+</p>
                                                    </div>
                            ...
                            <div className='w-full py-2'>
                                <button onClick={() => dispatch(resetCart())} className='px-10 py-3 bg-red-500 hover:bg-red-600 active:bg-red-500 text-white rounded-lg font-titleFont font-semibold text-lg tracking-wide'>Clear Cart</button>
                            </div>
                        </div>
                        <div className='w-full h-52 bg-white col-span-1 flex flex-col justify-center items-center p-4'>
                            <div>
                                <p className='flex gap-2 items-start text-sm'>
                                    <span><i className="fa-solid fa-circle-check bg-white text-green-500 rounded-full"></i>
                                    </span>{" "}
                                    Your order qualifies Free Shipping Choose this opyion at checkout. See details....
                                </p>
                            </div>
                            <div className='font-semibold px-10 py-1 flex items-center gap-2 justify-between'>
                                <p>Total: <span className='text-lg font-bold'>${totalPrice}</span></p>
                            </div>
                            <button className='w-full font-titleFont font-medium text-base bg-gradient-to-tr from-yellow-400 to-yellow-200 border hover:from-yellow-300 hover:to-yellow-500 border-yellow-500 hover:border-yellow-700 active:bg-gradient-to-bl active:from-yellow-400 active:to-yellow-500 duration-200 py-1.5 rounded-md mt-3'>Process to Pay</button>
                        </div>
                    </div>
                ) : (
                    <motion.div
                        ...
                        <div>
                            <img className='w-80 rounded-lg p-4 mx-auto' src={emtyCart} alt='no Product' />
                        </div>
                        <div className='w-96 p-4 bg-white flex flex-col items-center rounded-md shadow-lg'>
                            <h2 className='font-titleFont text-xl font-bold'>Your Cart feels lonely.</h2>
